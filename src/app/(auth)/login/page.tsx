...
function loginErrorMessage(message: string) {
  ...
  if (value.includes("email not confirmed")) {
    return "Primero confirma tu correo electrónico desde el mensaje que te enviamos.";
  }
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      ...
      if (data.session?.user) {
        // Supabase ya tiene una sesión válida. El dashboard decide
        // automáticamente si el usuario es personal o cliente.
  ...
  const handleLogin = async (event: React.FormEvent) => {
    ...
    // No hay una segunda pantalla ni un "redirect" intermedio.
    // Supabase autentica y el dashboard resuelve el acceso.
    window.location.replace("/dashboard");
  };
    ...
    <div className="min-h-screen bg-[#e5e0d7] flex flex-col items-center justify-center p-4 font-sans">
      <div className="w-full max-w-[420px] bg-[#f1eee7] rounded-[3rem] shadow-[0_24px_60px_rgba(0,0,0,0.16)] p-8 sm:p-10 border-2 border-[#d8d2c7]">
        <div className="text-center mb-10">
          ...
          <p className="mt-2 text-[10px] font-medium text-gray-700">
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase text-black ml-2">
              Correo
            </label>
          ...
          {(error ||
            ...
            queryError === "required" ||
            queryError === "system" ||
            queryError === "dashboard") && (
            <p
              role="alert"
              className="text-red-700 text-[11px] bg-red-50 border border-red-200 rounded-xl py-3 px-3 font-bold text-center"
            >
              {error ||
                (queryError === "session" || queryError === "required"
                  ? "Tu sesión no está disponible. Inicia sesión para continuar."
                  : "La sesión es válida, pero no pudimos abrir la información de la tienda. Inténtalo de nuevo.")}
            </p>
          )}
        ...
        <div className="mt-8 text-center">
          ...
          <Link
            href="/register"
            className="inline-block mt-2 text-xs font-black uppercase text-orange-600 hover:underline"
          >
